...
def grafica_curva_inflexion_vd1(df):
    ...
    # Solo datos sin iperf3 (red limpia) para la curva de inflexión
    if "red" in df.columns:
        df_filtrado = df[df["red"] == "limpia"]
    else:
        df_filtrado = df
    # Excluir filas donde VD1 es -1 (mediciones solo-vd2)
    df_filtrado = df_filtrado[df_filtrado[col] > 0]

    escenarios = sorted(df_filtrado["escenario"].unique())
    ...
    for esc in escenarios:
        df_esc = df_filtrado[df_filtrado["escenario"] == esc].copy()
...
def grafica_vd2_red(df):
    ...
    Genera una gráfica por escenario. Si hay datos de ambas redes (limpia y congestionada),
    muestra barras lado a lado. Si solo hay limpia, muestra solo esas.
    """
    col = "vd2_mediana_ms"
    if col not in df.columns:
    ...
    if tiene_red:
        # Solo usar sin_carga y congestionada para la gráfica comparativa
        df = df[df["red"].isin(["sin_carga", "congestionada"])].copy()
        estados_red = sorted(df["red"].unique())
        if not estados_red:
            print("  [SKIP] No hay datos sin_carga/congestionada para gráfica VD2.")
            return
    else:
        df = df.copy()
        df["red"] = "sin_carga"
        estados_red = ["sin_carga"]

    red_colors = {"sin_carga": "#55A868", "congestionada": "#C44E52", "limpia": "#55A868"}
    red_labels = {"sin_carga": "Sin iperf3 (red limpia)", "congestionada": "Con iperf3 (red congestionada)", "limpia": "Sin iperf3 (red limpia)"}
    ...
    for esc in sorted(df["escenario"].unique()):
        ...
        # Si hay datos congestionados, mostrar solo los hilos que tienen ambos
        if "congestionada" in estados_red:
            hilos_cong = set(df_esc[df_esc["red"] == "congestionada"]["n_hilos"].unique())
            hilos_limp = set(df_esc[df_esc["red"] == "sin_carga"]["n_hilos"].unique())
            hilos_disponibles = sorted(hilos_cong & hilos_limp)
        ...
        fig, ax = plt.subplots(figsize=(max(8, len(hilos_disponibles) * 2), 5.5))
        ...
        esc_txt = ESCENARIO_LABEL.get(esc, esc).replace("\n", " ")
        ...
        ax.spines[["top", "right"]].set_visible(False)
        fig.tight_layout()
        _guardar(fig, f"vd2_red_{esc}.png")
