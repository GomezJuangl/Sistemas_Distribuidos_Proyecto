...
class ServicioAnalitica:
    def __init__(self):
        ...
        self.PERSISTENCIA_PRINCIPAL_IP = "127.0.0.1"
        self.PERSISTENCIA_PRINCIPAL_PUERTO = 7001

        self.PERSISTENCIA_REPLICA_IP = "127.0.0.1"
        self.PERSISTENCIA_REPLICA_PUERTO = 7002

        
        # UMBRALES 
        self.umbral_volumen_congestion = 12
        self.umbral_velocidad_baja = 12
        self.umbral_vehiculos_espira = 100  
        #Diferencia para determinar cual eje esta congestionado
        ...
        # stubs / preparados para despues
        self.socket_push_control = None
        self.socket_push_persistencia_principal = None
        self.socket_push_persistencia_replica = None
    ...
    def conectar_broker(self):
        ...
        endpoint = f"tcp://{self.BROKER_IP}:{self.BROKER_PUERTO}"
        self.socket_sub_broker.connect(endpoint)
        
        self.socket_sub_broker.setsockopt_string(zmq.SUBSCRIBE, "")

        print(f"[ANALITICA] Conectada al broker en {endpoint}")
    ...
    def recibir_eventos(self):
        while True:
            ...
            self.enviar_persistencia_principal(interseccion, evento, estado_trafico, accion)
            self.enviar_persistencia_replica(interseccion, evento, estado_trafico, accion)
    ...
    def evaluar_estado_trafico(self, interseccion):
        ...
        # Decision final de congestion
        hay_congestion = volumen_alto or cola_alta or gps_alta or espira_alta
    ...
    def determinar_eje_congestionado(self, interseccion):
        ...
        if diferencia < self.umbral_diferencia_colas:
            return None #si la diferencia es muy pequeña no se asume ningun eje
    ...
    # SALIDAS (Envios a otros servicios o bdd)
    
    def enviar_comando_control(self, interseccion, accion):
        print(f"[CONTROL] Interseccion={interseccion} | Accion={accion}")

    def enviar_persistencia_principal(self, interseccion, evento, estado_trafico, accion):
        print(f"[PERSISTENCIA PRINCIPAL] {interseccion} | {estado_trafico} | {accion}")

    def enviar_persistencia_replica(self, interseccion, evento, estado_trafico, accion):
        print(f"[PERSISTENCIA REPLICA] {interseccion} | {estado_trafico} | {accion}")
    ...
    def imprimir_resumen(self, interseccion):
        ...
        if datos["camara"] is not None:
            cam = datos["camara"]
            print(
                f"Camara -> volumen={cam.get('volumen')} | "
                f"vel={cam.get('velocidad_promedio')} | "
                f"colaH={cam.get('cola_horizontal')} | "
                f"colaV={cam.get('cola_vertical')}"
            )

        if datos["gps"] is not None:
            gps = datos["gps"]
            print(
                f"GPS    -> vel={gps.get('velocidad_promedio')} | "
                f"nivel={gps.get('nivel_congestion')}"
            )

        if datos["espira"] is not None:
            esp = datos["espira"]
            print(
                f"Espira -> vehiculos={esp.get('vehiculos_contados')} | "
                f"intervalo={esp.get('intervalo_segundos')}"
            )
